...
import { Scale, TrendingUp, Dumbbell, Plus, X, Activity } from "lucide-react";
import "../fitness.css";
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl">
      <div className="page-header">
        <div>
          <h1 className="text-2xl font-display font-bold text-white mb-1">Progress</h1>
        ...
        <ScrollReveal variant="fade-up" className="mb-6">
          <div className="glass-card rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-display font-bold text-white">Log Body Measurements</h3>
              <button onClick={() => setShowAdd(false)} className="btn-press p-1.5 text-zinc-500 hover:text-zinc-300 rounded-lg hover:bg-zinc-800/50"><X size={16} /></button>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
              {["weight", "bodyFat", "chest", "waist", "hips", "arms", "thighs", "calves"].map(f => (
                <div key={f}>
                  <label className="text-[10px] text-zinc-500 font-mono uppercase tracking-wider">{f.replace(/([A-Z])/g, ' $1').trim()}</label>
            ...
            <button onClick={addMeasurement} className="w-full py-2.5 bg-emerald-500 hover:bg-emerald-600 text-black rounded-lg text-sm font-medium transition-all duration-200 btn-press">
              Save
            </button>
          </div>
        </ScrollReveal>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
                  ...
                  {volumeData.slice(-14).map((d, i) => {
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1 group relative">
                        <div className="w-full bg-gradient-to-t from-emerald-500/30 to-emerald-500/10 rounded-t relative" style={{ height: `${Math.max(h, 2)}%` }}>
                          <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] text-emerald-400 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap font-mono">
        ...
        <ScrollReveal variant="scale-in" stagger={50}>
          <div className="stat-card">
            <h3 className="text-sm font-display font-bold text-white mb-4 flex items-center gap-2">
              ...
              <div className="relative h-40">
                <div className="absolute inset-0">
                  <svg className="w-full h-full" viewBox={`0 0 ${weightData.length * 40} 160`} preserveAspectRatio="none">
                    ...
                    />
                    {weightData.map((d, i) => (
                      <circle key={i} cx={i * 40 + 20} cy={160 - ((d.weight - Math.min(...weightData.map(w => w.weight))) / (Math.max(...weightData.map(w => w.weight)) - Math.min(...weightData.map(w => w.weight)) || 1)) * 130 - 15} r="3.5" fill="#34d399" className="hover:r-5 transition-all" />
                    ))}
                  </svg>
                </div>
              </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                {prs.map((pr, i) => (
                  <div key={i} className="flex items-center justify-between py-2 border-b border-zinc-800/30 last:border-0">
                    <div>
                      <p className="text-sm text-white font-display font-semibold">{pr.exerciseName}</p>
                      <p className="text-[10px] text-zinc-500 capitalize">{pr.target}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-emerald-400 font-display font-bold">{pr.weight} kg</p>
        ...
        <ScrollReveal variant="fade-up" stagger={50}>
          <div className="stat-card">
            <h3 className="text-sm font-display font-bold text-white mb-4 flex items-center gap-2">
            ...
            {measurements.length === 0 ? (
              ...
              <div className="space-y-2">
                {measurements.slice(-5).reverse().map((m) => (
                  <div key={m.id} className="border-b border-zinc-800/30 last:border-0 pb-2">
                    <p className="text-xs text-zinc-500 mb-1.5 font-mono">{new Date(m.date).toLocaleDateString()}</p>
                    <div className="grid grid-cols-4 gap-2 text-xs">
                      {m.weight && <span className="text-zinc-400">Weight: <span className="text-white font-semibold">{m.weight}kg</span></span>}
                      {m.bodyFat && <span className="text-zinc-400">BF: <span className="text-white font-semibold">{m.bodyFat}%</span></span>}
                      {m.chest && <span className="text-zinc-400">Chest: <span className="text-white font-semibold">{m.chest}cm</span></span>}
                      {m.waist && <span className="text-zinc-400">Waist: <span className="text-white font-semibold">{m.waist}cm</span></span>}
                    </div>
                  </div>
                ))}
              </div>
